...
export default class RegistrationController extends BaseController {
  ...
  public async show(ctx: HttpContextContract) {
    return ctx.response.json(await this.getUser(ctx))
  }
  ...
  public async update(ctx: HttpContextContract) {
    ...
    try {
      ...
      const data = ctx.request.only(Object.keys(User.$keys.attributesToSerialized.all()))
  ...
  private async getUser(ctx) {
    ...
    const userData = ctx.auth.user?.serialize();
    // const profileData = await UserProfile.query().select('*').where('user_id' , userData.id)
    return { user : {...userData} , token: { type: type.toLowerCase(), token }}
  }
}
