...
class CommonRules {
  public static commonSchema() {
    return {
      industry : schema.string({ trim: true }, [rules.unique({ table: 'industries', column: 'industry' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'industries', column: 'value' })]),
    }
  }
