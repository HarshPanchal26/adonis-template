...
class CommonRules {
  public static commonSchema() {
    return {
      sector : schema.string({ trim: true }, [rules.unique({ table: 'sector_types', column: 'sector' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'sector_types', column: 'value' })]),
      industry_id : schema.number([ rules.unsigned()])
    }
  }
