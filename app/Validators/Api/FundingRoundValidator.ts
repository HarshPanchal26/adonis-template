...
class CommonRules {
  public static commonSchema() {
    return {
      round : schema.string({ trim: true }, [rules.unique({ table: 'funding_rounds', column: 'round' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'funding_rounds', column: 'value' })]),
    }
  }
