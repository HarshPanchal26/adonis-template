...
class CommonRules {
  public static commonSchema() {
    return {
      ...
      thought : schema.string({ trim: true }),
      is_thought_published_by_business : schema.boolean(),
      
      business_id : schema.number.nullable([rules.unsigned()]),
      likes : schema.number.nullableAndOptional([rules.unsigned()]),
      share : schema.number.nullableAndOptional([rules.unsigned()]),  
    }
  }
