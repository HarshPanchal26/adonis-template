...
class CommonRules {
  public static commonSchema() {
    return {
      stage : schema.string({ trim: true }, [rules.unique({ table: 'development_stages', column: 'stage' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'development_stages', column: 'value' })]),
    }
  }
