...
class CommonRules {
  public static commonSchema() {
    return {
      model : schema.string({ trim: true }, [rules.unique({ table: 'business_models', column: 'model' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'business_models', column: 'value' })]),
    }
  }
