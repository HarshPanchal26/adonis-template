...
class CommonRules {
  public static commonSchema() {
    return {
      audience : schema.string({ trim: true }, [rules.unique({ table: 'target_audiences', column: 'audience' })]),
      value : schema.string({ trim: true }, [rules.unique({ table: 'target_audiences', column: 'value' })]),
    }
  }
